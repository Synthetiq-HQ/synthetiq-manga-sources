...
(() => {
  ...
  function chapterFrom(chapter) {
    ...
    const rawChapter = attributes.chapter === null || attributes.chapter === undefined
      ? ""
      : String(attributes.chapter).trim();
    const number = numericOrNull(rawChapter);
    const chapterTitle = String(attributes.title || "").trim();
    let title;
    if (number !== null) {
      title = chapterTitle ? `Chapter ${number}: ${chapterTitle}` : `Chapter ${number}`;
    } else if (rawChapter) {
      // Non-numeric chapter labels (e.g. combined "15-16" uploads) keep their label.
      title = chapterTitle ? `Chapter ${rawChapter}: ${chapterTitle}` : `Chapter ${rawChapter}`;
    } else {
      title = chapterTitle || "Special";
    }
    return {
      id: href,
      href,
  ...
  function sortKeyFor(attributes) {
    // Null volume/chapter sort last so specials never jump ahead of numbered chapters.
    return {
      volume: numericOrNull(attributes.volume),
      chapter: numericOrNull(attributes.chapter),
    };
  }

  function chapterSort(a, b) {
    const aVolume = a.sortKey.volume;
    const bVolume = b.sortKey.volume;
    if (aVolume !== bVolume) {
      const aValue = aVolume === null ? Infinity : aVolume;
      const bValue = bVolume === null ? Infinity : bVolume;
      if (aValue !== bValue) return aValue - bValue;
    }
    const aChapter = a.sortKey.chapter;
    const bChapter = b.sortKey.chapter;
    const aValue = aChapter === null ? Infinity : aChapter;
    const bValue = bChapter === null ? Infinity : bChapter;
    if (aValue !== bValue) return aValue - bValue;
    const byDate = String(a.readableAt || "").localeCompare(String(b.readableAt || ""));
  ...
  async function extractChapters(id) {
    ...
    // never reorder or interleave chapters. Numbered chapters first, by volume
    // then chapter; unnumbered specials last.
    collected.sort(chapterSort);
    return collected.map(({ sortKey, ...mapped }) => mapped);
  }
  ...
  async function extractImages(id) {
    ...
    if (!hostname.endsWith("mangadex.network") || !hash) {
