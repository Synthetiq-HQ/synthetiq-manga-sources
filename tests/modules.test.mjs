...
test("KingOfShojo scopes search, chapters, and reader pages to the selected series", async () => {
  const fixtures = {
    home: await text("modules/kingofshojo/fixtures/home.html"),
    search: await text("modules/kingofshojo/fixtures/search.html"),
    searchPage2: await text("modules/kingofshojo/fixtures/search-page-2.html"),
    details: await text("modules/kingofshojo/fixtures/details.html"),
    chapter: await text("modules/kingofshojo/fixtures/chapter.html"),
    challenge: await text("modules/kingofshojo/fixtures/challenge.html"),
    emptyReader: await text("modules/kingofshojo/fixtures/empty-reader.html"),
    expected: await json("modules/kingofshojo/fixtures/expected.json"),
  };
  const calls = [];
  const module = await loadModule("modules/kingofshojo/index.js", {
    fetchv2: async (url, headers, method, body, options) => {
      assert.equal(method, "GET");
      assert.equal(body, null);
      calls.push({ url: String(url), headers, options });
      const parsed = new URL(url);
      if (parsed.pathname === "/page/2/") return response(fixtures.searchPage2);
      if (parsed.pathname === "/" && parsed.searchParams.get("s") === "challenge") return response(fixtures.challenge);
      if (parsed.pathname === "/") {
        if (parsed.searchParams.has("s")) return response(fixtures.search);
        return response(fixtures.home);
      }
      if (parsed.pathname === "/manga/fixture-chronicle/") return response(fixtures.details);
      if (parsed.pathname === "/empty-chapter-1/") return response(fixtures.emptyReader);
      if (/-chapter-[0-9]+(?:\.[0-9]+)?\/$/i.test(parsed.pathname)) return response(fixtures.chapter);
      throw new Error(`Unexpected KingOfShojo fixture URL: ${url}`);
    },
  });

  const manifest = await json("modules/kingofshojo/manifest.json");
  assert.deepEqual(manifest.allowedHosts, ["kingofshojo.com", "cdn.kingofshojo.com", "i1.wp.com", "i2.wp.com"]);
  assert.deepEqual(JSON.parse(JSON.stringify(await module.searchResults("fixture", 1))), fixtures.expected.search);
  assert.deepEqual(JSON.parse(JSON.stringify(await module.searchResults("fixture", 2))), fixtures.expected.searchPage2);
  await assert.rejects(() => module.searchResults("challenge", 1), /browser challenge/i);

  const fallbackModule = await loadModule("modules/kingofshojo/index.js", {
    fetchv2: async (url) => {
      const parsed = new URL(url);
      if (parsed.pathname === "/" && parsed.searchParams.get("s") === "father i dont want to get married") {
        return response("<html><body><h1>Search Results</h1><p>No results.</p></body></html>");
      }
      if (parsed.pathname === "/" && parsed.searchParams.get("s") === "father i don t") return response(fixtures.search);
      return response(fixtures.home);
    },
  });
  const fallbackSearch = await fallbackModule.searchResults("father i dont want to get married", 1);
  assert.equal(fallbackSearch.items[0].title, "Fixture Chronicle");

  const details = await module.extractDetails(fixtures.expected.details.id);
  assert.deepEqual(JSON.parse(JSON.stringify(details)), fixtures.expected.details);
  const chapters = await module.extractChapters(details.id);
  assert.deepEqual(JSON.parse(JSON.stringify(chapters)), fixtures.expected.chapters);
  assert.deepEqual(JSON.parse(JSON.stringify(chapters.map((chapter) => chapter.number))), [1, 3.5, 12]);
  assert.equal(chapters.some((chapter) => chapter.id.includes("other-title")), false);

  const pages = await module.extractImages(chapters[0].id);
  assert.deepEqual(JSON.parse(JSON.stringify(pages)), fixtures.expected.images);
  assert.equal(pages.some((page) => page.url.includes("i.ibb.co")), false);
  assert.ok(pages.every((page) => page.headers.Referer === chapters[0].id));
  await assert.rejects(
    () => module.extractImages("https://kingofshojo.com/empty-chapter-1/"),
    /no readable page images/i,
  );
  await assert.rejects(
    () => module.extractDetails("https://cdn.kingofshojo.com/manga/fixture-chronicle/"),
    /outside the source host/i,
  );

  const discovery = await module.discoveryHome();
  assert.equal(discovery.sections.length, 1);
  assert.ok(discovery.sections[0].items.some((item) => item.title === "Fixture Companion"));
  assert.ok(calls.some((call) => call.url === "https://kingofshojo.com/"));
  assert.ok(calls.some((call) => call.url.includes("?s=fixture")));
});
