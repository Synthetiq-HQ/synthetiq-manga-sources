...
test("MangaXo follows public AJAX chapter and image manifests safely", async () => {
  const fixtures = {
    home: await text("modules/mangaxo/fixtures/home.html"),
    search: await text("modules/mangaxo/fixtures/search.html"),
    searchPage2: await text("modules/mangaxo/fixtures/search-page-2.html"),
    details: await text("modules/mangaxo/fixtures/details.html"),
    readingList: await text("modules/mangaxo/fixtures/reading-list.json"),
    images: await text("modules/mangaxo/fixtures/images.json"),
    invalidImages: await text("modules/mangaxo/fixtures/images-invalid.json"),
    emptyImages: await text("modules/mangaxo/fixtures/images-empty.json"),
    expected: await json("modules/mangaxo/fixtures/expected.json"),
  };
  const calls = [];
  const module = await loadModule("modules/mangaxo/index.js", {
    fetchv2: async (url, headers, method, body, options) => {
      assert.equal(method, "GET");
      assert.equal(body, null);
      ...
      if (parsed.pathname === "/search") {
        if (parsed.searchParams.get("keyword") === "challenge") return response("<title>Just a moment...</title>");
        if (parsed.searchParams.get("page") === "2") return response(fixtures.searchPage2);
        return response(fixtures.search);
      }
      if (parsed.pathname === "/home") return response(fixtures.home);
      if (parsed.pathname === "/manga/fixture-chronicle") return response(fixtures.details);
      if (parsed.pathname === "/ajax/reading-list/4242") return response(fixtures.readingList);
      if (parsed.pathname === "/ajax/image/list/9002") return response(fixtures.invalidImages, 202);
      if (parsed.pathname === "/ajax/image/list/9004") return response(fixtures.emptyImages, 202);
      if (/^\/ajax\/image\/list\/\d+$/.test(parsed.pathname)) return response(fixtures.images, 202);
      throw new Error(`Unexpected MangaXo fixture URL: ${url}`);
    },
  });

  const manifest = await json("modules/mangaxo/manifest.json");
  ...
  const details = await module.extractDetails("https://mangaxo.com/manga/fixture-chronicle");
  assert.deepEqual(JSON.parse(JSON.stringify(details)), fixtures.expected.details);
  const chapters = await module.extractChapters(details.id);
  assert.deepEqual(JSON.parse(JSON.stringify(chapters)), fixtures.expected.chapters);
  assert.deepEqual(JSON.parse(JSON.stringify(chapters.map((chapter) => chapter.number))), [1, 1.5, 2, 3]);
  assert.equal(chapters.some((chapter) => chapter.id.includes("other-series")), false);

  const pages = await module.extractImages(chapters[0].id);
  assert.deepEqual(JSON.parse(JSON.stringify(pages)), fixtures.expected.images);
  assert.ok(pages.every((page) => page.headers.Referer === chapters[0].id));
  await assert.rejects(
    () => module.extractImages("https://mangaxo.com/manga/fixture-chronicle/chapter-2"),
    /invalid or duplicate page manifest/i,
  );
  await assert.rejects(
    () => module.extractImages("https://mangaxo.com/manga/fixture-chronicle/chapter-1.5"),
    /no readable page images/i,
  );
  await assert.rejects(
    () => module.extractDetails("https://i2.wp.com/mangaxo.com/manga/fixture-chronicle"),
    /outside the source host/i,
  );

  const discovery = await module.discoveryHome();
  assert.deepEqual(JSON.parse(JSON.stringify(discovery)), fixtures.expected.discovery);
  assert.ok(calls.some((call) => call.url === "https://mangaxo.com/ajax/reading-list/4242"));
  assert.ok(calls.some((call) => call.url.includes("/ajax/image/list/9001")));
});
